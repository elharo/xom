...
class JaxenNavigator extends DefaultNavigator {
    ...
    public Iterator getDescendantAxisIterator(Object contextNode) {
        ...
        if (contextNode instanceof ParentNode) {
            return getDescendants((ParentNode) contextNode).iterator();
        }
        else {
    ...
    public Iterator getDescendantOrSelfAxisIterator(Object contextNode) {
        ...
        if (contextNode instanceof ParentNode) {
            List descendants = getDescendants((ParentNode) contextNode);
            ArrayList result = new ArrayList(descendants.size() + 1);
            result.add(contextNode);
            result.addAll(descendants);
            return result.iterator();
        }
        else {
            return getSelfAxisIterator(contextNode); 
        }
    ...
    private static List getDescendants(ParentNode root) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.isText()) {
                XOMList texts = new XOMList();
                texts.add(child);
                while (true) {
                    int next = i+1;
                    if (next == childCount) break;
                    Node nextNode = root.getChild(next);
                    if (nextNode.isText()) {
                        texts.add(nextNode);
                        i++;
                    }
            ...
            if (child.isElement() || child.isDocument()) {
                // recursion????
                result.addAll(getDescendants((ParentNode) child));
            }
    ...
    /* private static class DescendantAxisIterator implements Iterator {

        private ParentNode parent = null;
        private ParentNode original;
        private Node current;
        private int currentPosition = -1;
        
        DescendantAxisIterator(ParentNode node) {
            this.original = node;
            this.current = node;
        }
